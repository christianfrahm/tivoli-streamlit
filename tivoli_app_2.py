...
if st.session_state.step == 1:
    ...
    if st.session_state.artist_index < len(artists):
        ...
        hot = st.button("🔥 Hot")
        not_hot = st.button("❌ Not")
        if hot or not_hot:
            st.session_state.artist_hot[artist] = 1 if hot else 0
    ...
    if st.session_state.performance_index < len(performances):
        ...
        st.subheader(f"🎭 {show}")
        interested = st.button("👍 Interesseret")
        not_interested = st.button("👎 Ikke mig")
        if interested or not_interested:
            st.session_state.performance_hot[show] = 1 if interested else 0
