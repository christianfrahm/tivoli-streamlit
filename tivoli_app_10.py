...
# Helper: Show centered full-width button
...
st.markdown("<h1 style='text-align: center;'>🎡 Din Tivoli-profil</h1>", unsafe_allow_html=True)

# Step 1
if st.session_state.step == 1:
    st.markdown("### Spørgsmål 1", unsafe_allow_html=True)
...
# Step 2
    ...
    st.markdown("### Spørgsmål 2", unsafe_allow_html=True)
...
# Step 21 – Artist Preference
    ...
    if st.session_state.artist_index < len(artists):
        ...
        st.markdown(f"### 🎤 {artist['name']}", unsafe_allow_html=True)
        show_image(artist["img"])
        if centered_button("🔥 Hot", key=f"hot_{artist['name']}"):
            st.session_state.artist_hot[artist["name"]] = 1
            st.session_state.artist_index += 1
        elif centered_button("❌ Not", key=f"not_{artist['name']}"):
...
# Step 3 – Food
    ...
    st.markdown("### Spørgsmål 3", unsafe_allow_html=True)
...
# Step 4 – Rides
    ...
    st.markdown("### Spørgsmål 4", unsafe_allow_html=True)
...
# Step 5 – Show Interest
    ...
    st.markdown("### Spørgsmål 5", unsafe_allow_html=True)
...
# Step 51 – Show Preferences
    ...
    if st.session_state.performance_index < len(performances):
        ...
        st.markdown(f"### 🎭 {perf['name']}", unsafe_allow_html=True)
        show_image(perf["img"])
        if centered_button("👍 Interesseret", key=f"inter_{perf['name']}"):
            st.session_state.performance_hot[perf["name"]] = 1
            st.session_state.performance_index += 1
        elif centered_button("👎 Ikke mig", key=f"not_inter_{perf['name']}"):
...
# Step 6 – Output
    ...
    st.markdown("### 🎯 Din Tivoli-profil", unsafe_allow_html=True)
    tags = []
    ...
    st.markdown(f"<p style='text-align: center;'>{', '.join(tags) if tags else 'Ingen relevante segmenter endnu.'}</p>", unsafe_allow_html=True)
