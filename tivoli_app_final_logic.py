...
if st.session_state.step == 1:
    ...
    st.session_state.besoeg = st.radio("Hvor mange gange tror du, du kommer til at besøge Tivoli i år?", [
    ...
    st.session_state.koncert = st.radio("Er du typen, der nyder en god koncert i Tivoli?", [
        "Ja, helt sikkert", "Af og til", "Nej, det er ikke lige mig"
    ])
    if st.button("Næste"):
        if st.session_state.koncert in ["Ja, helt sikkert", "Af og til"]:
    ...
    st.session_state.mad = st.radio("Hvilken type madoplevelser er du mest interesseret i i Tivoli?", [
    ...
    st.session_state.forlystelser = st.radio("Hvor meget betyder forlystelser for dit besøg i Tivoli?", [
    ...
    st.session_state.forestillinger = st.radio("Er du interesseret i forestillinger som teater, dans eller musicals i haven?", [
        "Ja, det er noget af det bedste", "Nogle gange, hvis det passer ind", "Nej, det er ikke lige mig"
    ])
    if st.button("Næste"):
        if st.session_state.forestillinger in ["Ja, det er noget af det bedste", "Nogle gange, hvis det passer ind"]:
